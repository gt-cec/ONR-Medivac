...
def makeMonitorFigure(plot_mapping):
    ...
    plot_mapping_colors={'wave_ecg':"#00FC00", 'wave_pleth':"#00FCFF", 'wave_nibp':"#FC0000", 'wave_etco2':"#FF78FF"}
    plt.close('all')
    plt.rcParams.update({
    "lines.color": "white",
...
def checkPatientStatus():
    ...
    vitals_state = time_sec%3
    print(time_sec)
    print('vitals_state = ',vitals_state)
    print('')
    try:
        response = http.request('GET', 'http://192.168.1.100:8080/var', timeout=0.25, retries=False)
...
#plt.gcf().canvas.set_window_title('MK217 Patient Monitor')  # did not work on Xubuntu (since that uses GTK4Agg)
