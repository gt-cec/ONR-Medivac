...
def inflight_status_check(inflight_event,status_report_event,emergency_event,administer_event,response_event,takeoff_event,tank_event, engine_event):
   while True:
      print("IN",inflight_event.is_set())
      print("EM:",emergency_event.is_set())
      if not emergency_event.is_set() and inflight_event.is_set():
...
def main(inflight_event, status_report_event,emergency_event,administer_event,response_event,takeoff_event,tank_event, engine_event):
...
def set_radio_update():  # to manually trigger the radio update complete
    print("Radio Updated completed")
...
event_handlers = {
    ...
    "status_report":inflight_status_check,
...
if __name__ == "__main__":
   ...
   while True:
        states = fetch_states()
        if states:
            for event, is_set in states.items():
